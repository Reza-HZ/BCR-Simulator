...
class BCRSimulator:
    ...
    def _initialize_vdj_segments(self):
        ...
        segments = {
            "V": read_fasta(r"K:\BCellPhylo\ClusteringBCtrees\sim2\IGH genes\V.fasta"),
            "D": read_fasta(r"K:\BCellPhylo\ClusteringBCtrees\sim2\IGH genes\D.fasta"),
            "J": read_fasta(r"K:\BCellPhylo\ClusteringBCtrees\sim2\IGH genes\J.fasta")
        }
        return segments
    ...
    def generate_repertoire_dynamically(self, pathogen, align_type = 'a', affinity_threshold=0.2, 
                                        max_generations=8, mutation_rate=0.001, max_abundancy = 100):
        root = self._generate_initial_bcr()
        root["affinity"] = pathogen.calculate_affinity_n(root["sequence"]) if align_type == 'n' else pathogen.calculate_affinity_a(root["a_sequence"])
        if root["affinity"] < 0.3:
            root["abundance"] = 1
        else:
            expected_abundance = round(max_abundancy ** (root["affinity"] ** 3))
            abundance = np.random.poisson(expected_abundance)
            root["abundance"] = max(abundance, 1)
        ...
        while generation <= max_generations:
            ...
            for parent in current_generation:
                ...
                while i < parent_abd:
                    ...
                    affinity = pathogen.calculate_affinity_n(mutated_seq) if align_type == 'n' else pathogen.calculate_affinity_a(mutated_a_seq)
                    ...
                    expected_abundance = round(max_abundancy ** (affinity ** 3))
                    abundance = np.random.poisson(expected_abundance)
                    abundance = max(abundance, 1)
...
def export_to_fasta(bcrs, filename):
    with open(filename, 'w') as f:
        for b in bcrs:
            f.write(f">{b['id']}|abundance={b['abundance']}\n{b['sequence']}\n")
...
if __name__ == "__main__":
    ...
    with open(r'K:\BCellPhylo\ClusteringBCtrees\sim2\IGH genes\epitope.txt', 'r') as file:
        epitope_list = [line.strip() for line in file.readlines()]
    

    n_epitopes = random.randint(10, 20)
    ...
    while(current_run <= n_runs):
        ...
        if stats['num_unique_sequences'] > 15:
            ...
            newick = export_to_newick(merged_repertoire, os.path.join(run_folder, "repertoire.newick"))
